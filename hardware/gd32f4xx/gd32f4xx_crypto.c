...
#include "hardware/gd32f4xx/gd32f4xx_crypto_hash.h"
#include "hardware/gd32f4xx/gd32f4xx_crypto_cipher.h"
#include "debug.h"
...
#if (GD32F4XX_CRYPTO_HASH_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
...
#if (GD32F4XX_CRYPTO_CIPHER_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
